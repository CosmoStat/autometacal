...
_pvals_gauss = tf.Variable([1.0])
_fvals_gauss = tf.Variable([1.0])
...
class GMix(object):
        ...
        for i in range(gm.size):
            ...
            if det > gmix_nb.GMIX_LOW_DETVAL:
                ...
                try:
                    ...
                    newmat = tf.linalg.inv(invsum)
                    newdet = tf.linalg.det(newmat)

                    fac = tf.math.sqrt(newdet / det)
                    if fac > 1:
                        fac = 1
    ...
    def make_image(self, dims, jacobian=None, fast_exp=False):
        ...
        dims = tf.convert_to_tensor(dims, dtype="i8")
        if len(dims) != 2:
            raise ValueError(
                "images must have two dimensions, " "got %s" % str(dims)
            )

        image = tf.zeros(dims, dtype="f8")
    ...
    def get_weighted_sums(self, obs, maxrad, res=None):
        ...
        if res is None:
            dt = tf.dtype(_moments_result_dtype, align=True)
    ...
    def reset(self):
        ...
        """
        self._pars = tf.Variable(tf.zeros(self._npars))
        self._data = tf.Variable(tf.zeros(self._ngauss))#, dtype=_gauss2d_dtype)
...
def get_weighted_moments_stats(ares):
    ...
    res["T"] = -9999.0
    res["s2n"] = -9999.0
    res["e"] = tf.convert_to_tensor([-9999.0, -9999.0])
    res["e_err"] = tf.convert_to_tensor([9999.0, 9999.0])
    res["e_cov"] = tf.diag([9999.0, 9999.0])

    fvar_sum = sums_cov[5, 5]

    if fvar_sum > 0.0:

        res["flux_err"] = tf.math.sqrt(fvar_sum)
    ...
    if res["flags"] == 0:
        ...
        if flux_sum > 0.0:
            ...
            if res["T"] > 0.0:
                ...
                if tf.math.is_inf(e1_err) and tf.math.is_inf(e2_err):
                    res["e_cov"] = tf.diag([e1_err ** 2, e2_err ** 2])
                    res["e_err"] = tf.convert_to_tensor([e1_err, e2_err])
