...
class GalGenConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self, *,dataset_size=100000, stamp_size=51, pixel_scale=.263, **kwargs):
    ...
    super(GalGenConfig, self).__init__(
        description=("%d stamps of %d x %d pixels, %.2f arcsec/pixel, flux = 1." %
...
def gs_generate_images(**kwargs):
  ...
  Generates noiseless galaxy images with a simple light profile. 
  The resulting image is before the convolution with a PSF. 
  ...
  #ellipticity range
  a = -defaults['g_range'] / defaults['g_scatter']
  b =  defaults['g_range'] / defaults['g_scatter']
